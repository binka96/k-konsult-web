    ...
    this.propertyService.getListofImages(this.article.title).subscribe({
      next: (response)=>{
        ...
        for (let i = 0; i < response.length; i++) {
          this.images.push({ 
             previewImageSrc: "http://localhost:8080/K-Konsult/file/Get/images/"+this.article.title+"/"+ response[i], 
             thumbnailImageSrc:  "http://localhost:8080/K-Konsult/file/Get/images/"+this.article.title+"/"+ response[i], 
