      ...
      this.articleService.getArticleByTitle(this.selectedArticles.name).subscribe(
        ...
        this.propertyService.getListofImages(this.selectedArticles.name).subscribe({
          next: (response)=>{
            ...
            for (let i = 0; i < response.length; i++) {
              this.images.push({ 
                 previewImageSrc: "http://localhost:8080/K-Konsult/file/Get/images/"+this.selectedArticles.name+"/"+ response[i], 
                 thumbnailImageSrc:  "http://localhost:8080/K-Konsult/file/Get/images/"+this.selectedArticles.name+"/"+ response[i], 
